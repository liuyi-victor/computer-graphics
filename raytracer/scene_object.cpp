...
bool UnitSquare::intersect( Ray3D& ray, const Matrix4x4& worldToModel,
	...
	else
	{
		...
		double t_value = (world_space_poi[0] - ray.origin[0]) / ray.dir[0];
			...
			const Matrix4x4& modelToWorld, Material* mat )
{
	...
	double vx = trans_dir[0];
	double vy = trans_dir[1];
	double vz = trans_dir[2];

	double ex = trans_origin[0];
	double ey = trans_origin[1];
	double ez = trans_origin[2];
	...
	if(t1 >= 0.0 && t2 >= 0.0)
	{
		...
		world_space_normal.normalize();
		ray.intersection.normal = world_space_normal;
		ray.intersection.t_value = t_value;//(ray.intersection.point[0] - ray.origin[0]) / ray.dir[0];
		ray.intersection.none = false;
