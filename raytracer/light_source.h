...
public:
	...
	_col_diffuse(col), _col_specular(col) { point = 0; }
	AreaLight( Point3D pos, Colour ambient, Colour diffuse, Colour specular ) 
	: _pos(pos), _col_ambient(ambient), _col_diffuse(diffuse), 
	_col_specular(specular) {  point = 0; }


	AreaLight( Point3D pos, Colour col, int w, int h ) : _pos(pos), _col_ambient(col), 
	_col_diffuse(col), _col_specular(col) { width = w; height = h;  point = 0; }


	void shade( Ray3D& ray);
	void shadePoint( Ray3D& ray, Point3D& point);
	Point3D get_position() const { return _pos; }
	int width;
	int height;
