...
void Raytracer::computeShading( Ray3D& ray ) {
	...
	if(curLight->light->point == 1)
	{
		...
		if (!check_shadow.intersection.none) 
		{
			...
			{
				ray.col = ray.col + ray.intersection.mat->ambient;
				ray.col.clamp();
			}
		} 
		else 
	...
	{
		int i;
		AreaLight* source = (AreaLight*)curLight->light;
		for(i = 0; i < source->width * source->height ; i++)
		{
			double randx = ((double)rand() / RAND_MAX) * source->width;
			double randy = ((double)rand() / RAND_MAX) * source->height;

			Point3D center = curLight->light->get_position();
			Point3D light_pos(center[0] + randx, center[1] + randy, center[2]);
			Vector3D light_dir = light_pos - ray.intersection.point;
			double light_distance = light_dir.length();
			light_dir.normalize();
			Ray3D check_shadow(ray.intersection.point, light_dir);
			...
			{
				source->shadePoint(ray, light_pos);
			}
		}
	}
