...
function drawPolygon(ctx, poly)
{
	...
	drawLineSegment(ctx, poly[len-1][0], poly[len-1][1], poly[0][0], poly[0][1]);
}
...
function drawCircle(ctx, cx, cy, r)
{
	...
	for(var i = 0; i < 36; i++)
	{
		drawLineSegment(ctx, r*Math.cos(2*Math.PI*i/36)+cx, r*Math.sin(2*Math.PI*i/36)+cy, r*Math.cos(2*Math.PI*(i+1)/36)+cx, r*Math.sin(2*Math.PI*(i+1)/36)+cy);
	}
	//drawLineSegment(ctx, Math.cos(2*Math.PI*35/36)+cx, Math.sin(2*Math.PI*35/36)+cy, Math.cos(0)+cx, Math.sin(0)+cy);
}
...
function rotationAboutPoint(angle, joint)
{
	...
	T[0][1] = Math.sin(angle)*(-1);
	T[0][2] = joint[0]-joint[0]*Math.cos(angle)+joint[1]*Math.sin(angle);
	T[1][0] = Math.sin(angle);
	T[1][1] = Math.cos(angle);
	T[1][2] = joint[1]-joint[0]*Math.sin(angle)-joint[1]*Math.cos(angle);
	return T;
}
...
function drawPenguin(ctx)
{
  ...
  var arm_offset = [30, -10]; // The location of the arm relative to the body
  var arm_joint_offset = [0, -60]; // The location of the arm joint relative to the head center
  ...
  var leg1_offset = [-57, 190]; // The location of the left leg relative to the body
  var leg1_joint_offset = [0, -35]; // The location of the left hip joint relative to the left leg center
  var leg1_T = composeTransforms(
    rotationAboutPoint(hip_angles[0], leg1_joint_offset),
    translateByOffset(leg1_offset)
  );
 
  // Transform and draw the left in a hierarchical fashion
  // That is, if the body moves, then the left leg will move with it.
  var leg1_poly = transformPolygon(leg_poly, leg1_T);
  leg1_poly = transformPolygon(leg1_poly, torso_T);
  drawPolygon(ctx, leg1_poly);
  // Now draw the left hip joint
  hip_joints[0] = transformPoint(leg1_joint_offset, leg1_T);
  ...
  var leg_poly2 = [[-15, -53], [-20, 52], [ 20,  52], [ 12, -53]];
  var leg2_offset = [57, 193]; // The location of the right leg relative to the body
  var leg2_joint_offset = [0, -35]; // The location of the right hip joint relative to the right leg center
  ...
  // Transform and draw the left in a hierarchical fashion
  // That is, if the body moves, then the left leg will move with it.
  var leg2_poly = transformPolygon(leg_poly2, leg2_T);
  ...
  var foot1_offset = [-50, 45]; // The location of the left foot relative to the left leg
  var foot1_joint_offset = [50, 0]; // The location of the left an joint relative to the left foot center
  var foot1_T = composeTransforms(
    rotationAboutPoint(ankle_angles[0], foot1_joint_offset),
    translateByOffset(foot1_offset)
  );
 
  // Transform and draw the left foot in a hierarchical fashion
  // That is, if the body or left leg moves, then the left foot will move with it.
  var foot1_poly = transformPolygon(foot_poly, foot1_T);
  foot1_poly = transformPolygon(foot1_poly, leg1_T);
  foot1_poly = transformPolygon(foot1_poly, torso_T);
  drawPolygon(ctx, foot1_poly);
  // Now draw the left hip joint
  ankle_joints[0] = transformPoint(foot1_joint_offset, foot1_T);
  ankle_joints[0] = transformPoint(ankle_joints[0], leg1_T);
  ...
  var foot2_offset = [-50, 45]; // The location of the right foot relative to the right leg
  var foot2_joint_offset = [50, 0]; // The location of the right an joint relative to the right foot center
  var foot2_T = composeTransforms(
    rotationAboutPoint(ankle_angles[1], foot2_joint_offset),
    translateByOffset(foot2_offset)
  );
 
  // Transform and draw the right foot in a hierarchical fashion
  // That is, if the body or right leg moves, then the right foot will move with it.
  ...
  // Now draw the right hip joint
  ankle_joints[1] = transformPoint(foot2_joint_offset, foot2_T);
  ...
  // Define the transformation for the upper beak as a rotation and then a translation
  var upperbeak_offset = [-90, 0]; // The location of the upper beak relative to the head
  var upperbeak_T = translateByOffset(upperbeak_offset);
 
  // Transform and draw the upper beak in a hierarchical fashion
  // That is, if the body or head moves, then the upper will move with it.
  upper_beak_poly = transformPolygon(upper_beak_poly, upperbeak_T);
  ...
  // Define the transformation for the lower beak as a rotation and then a translation
  var lowerbeak_offset = [-90, 30]; // The location of the lower beak relative to the head
  var lowerbeak_T = composeTransforms(
    translateByOffset([0, mouth_gap]),
    translateByOffset(lowerbeak_offset)
  );
 
  // Transform and draw the lower beak in a hierarchical fashion
  // That is, if the body or head moves, then the lower will move with it.
  lower_beak_poly = transformPolygon(lower_beak_poly, lowerbeak_T);
  ...
  // Define the transformation for the eye and iris as a rotation and then a translation
  var eye_offset = [-33, -17]; // The location of the eye relative to the head
  var eye_T = translateByOffset(eye_offset);
 
  // Transform and draw the eye and iris in a hierarchical fashion
  // That is, if the body or head moves, then the eye and iris will move with it.
